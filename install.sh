...
#
# To run:
# curl -fsSL "https://l30pems0e3ubg.pit-1.try.coder.app/install.sh" | sh

usage() {
	arg0="$0"
	if [ "$0" = sh ]; then
		arg0="curl -fsSL \"https://l30pems0e3ubg.pit-1.try.coder.app/install.sh\" | sh -s --"
	else
		not_curl_usage="The latest script is available at https://l30pems0e3ubg.pit-1.try.coder.app/install.sh
"
	fi
...
Coder v2.24.1+f97bd76 installed.
...
main() {
	...
	ORIGIN="https://l30pems0e3ubg.pit-1.try.coder.app"
	ALL_FLAGS=""
...
install_standalone() {
	echoh "Installing coder-$OS-$ARCH v2.24.1+f97bd76 from $ORIGIN."
	echoh

	BINARY_FILE="$CACHE_DIR/coder-${OS}-${ARCH}-v2.24.1+f97bd76"
